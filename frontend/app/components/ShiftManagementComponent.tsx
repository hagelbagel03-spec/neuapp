...
const ShiftManagementComponent = ({ user, token, API_URL, colors, isDarkMode, isSmallScreen, isMediumScreen }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // ✅ NEU: Load sick leaves
      try {
        const sickLeavesResponse = await axios.get(`${API_URL}/api/sick-leave`, config);
        if (sickLeavesResponse.data) {
          console.log('🏥 ShiftManagement: Loaded my sick leaves:', sickLeavesResponse.data);
          setSickLeaves(sickLeavesResponse.data || []);
        }
      } catch (error) {
        console.log('Krankmeldungen laden fehlgeschlagen:', error.message);
        setSickLeaves([]);
      }
    } catch (error) {
  ...
  // ✅ NEU: Krankmeldung einreichen
  const requestSickLeave = async () => {
    if (!sickLeaveFormData.start_date || !sickLeaveFormData.end_date || !sickLeaveFormData.reason) {
      Alert.alert('❌ Fehler', 'Bitte alle Felder ausfüllen.');
      return;
    }
    ...
    try {
      ...
      const sickLeaveData = {
        start_date: sickLeaveFormData.start_date,
        end_date: sickLeaveFormData.end_date,
        reason: sickLeaveFormData.reason,
        medical_certificate: sickLeaveFormData.medical_certificate
      };

      const response = await axios.post(`${API_URL}/api/sick-leave`, sickLeaveData, config);
      
      if (response.data) {
        Alert.alert('✅ Erfolg', 'Krankmeldung wurde eingereicht!');
        
        setSickLeaveFormData({ start_date: '', end_date: '', reason: '', medical_certificate: false });
        setShowSickLeaveModal(false);
        
        console.log('🔄 Lade Krankmeldungen neu...');
        await loadData();
      }
    } catch (error) {
      console.error('❌ Sick leave request error:', error);
      Alert.alert('❌ Fehler', 'Krankmeldung konnte nicht eingereicht werden.');
    }
  };
  ...
  const dynamicStyles = StyleSheet.create({
    ...
    // ✅ WARMES DESIGN für Schichtverwaltung
    modernSectionHeader: {
      flexDirection: 'row',
      alignItems: 'center',
      marginBottom: 20,
    ...
    <ScrollView style={dynamicStyles.container}>
      ...
      <View style={dynamicStyles.section}>
        ...
        {vacations.length === 0 ? (
          ...
          vacations.slice(0, 3).map((vacation, index) => (
              ...
              <View style={dynamicStyles.modernVacationHeader}>
                ...
                <View style={dynamicStyles.vacationMainContent}>
                  <View style={dynamicStyles.vacationTopRow}>
                    ...
                    <View style={[dynamicStyles.modernStatusChip, { 
                  ...
                  {vacation.reason && (
                    <View style={dynamicStyles.vacationReasonContainer}>
                      ...
                      <Text style={dynamicStyles.modernVacationReason}>
                        {vacation.reason}
                      </Text>
                    </View>
                  )}
                  ...
                  {vacation.status === 'rejected' && vacation.rejection_reason && (
                    <View style={dynamicStyles.rejectionReasonContainer}>
                      ...
                      <Text style={dynamicStyles.rejectionReasonText}>
      ...
      {/* ✅ NEU: Meine Krankmeldungen - genau wie Urlaubsanträge */}
      <View style={dynamicStyles.section}>
        <View style={dynamicStyles.modernSectionHeader}>
          <View style={dynamicStyles.sectionIconContainer}>
          ...
          <View style={dynamicStyles.sectionTextContainer}>
            <Text style={dynamicStyles.modernSectionTitle}>Meine Krankmeldungen</Text>
            <Text style={[dynamicStyles.modernSectionSubtitle, { color: colors.text, opacity: 0.8 }]}>Status und Verwaltung</Text>
          </View>
          <TouchableOpacity 
            style={dynamicStyles.modernQuickButton}
        ...
        {sickLeaves.length === 0 ? (
          <View style={dynamicStyles.emptyContainer}>
            ...
            <Text style={dynamicStyles.emptyText}>Keine Krankmeldungen vorhanden</Text>
          </View>
        ) : (
          sickLeaves.slice(0, 3).map((sickLeave, index) => (
            <View key={sickLeave.id || index} style={dynamicStyles.modernVacationCard}>
              <View style={dynamicStyles.modernVacationHeader}>
                <View style={dynamicStyles.vacationIconContainer}>
                  <View style={[dynamicStyles.vacationStatusDot, { backgroundColor: getStatusColor(sickLeave.status) }]}>
                    <Ionicons 
                      name={sickLeave.status === 'approved' ? 'checkmark' : 
                            sickLeave.status === 'rejected' ? 'close' : 
                            'time'}
                      size={16} 
                      color="#FFFFFF" 
                    />
                ...
                <View style={dynamicStyles.vacationMainContent}>
                  <View style={dynamicStyles.vacationTopRow}>
                    ...
                    <View style={[dynamicStyles.modernStatusChip, { 
                      backgroundColor: getStatusColor(sickLeave.status) + '20', 
                      borderColor: getStatusColor(sickLeave.status) 
                    }]}>
                      <Text style={[dynamicStyles.modernStatusChipText, { color: getStatusColor(sickLeave.status) }]}>
                  ...
                  {sickLeave.medical_certificate && (
                    <View style={[dynamicStyles.vacationReasonContainer, { marginTop: 8 }]}>
                      <Ionicons name="document-attach-outline" size={14} color={colors.success} />
                      <Text style={[dynamicStyles.modernVacationReason, { color: colors.success }]}>
                        Attest vorhanden
                      </Text>
                    </View>
                  )}

                  {sickLeave.status === 'rejected' && sickLeave.rejection_reason && (
                    <View style={dynamicStyles.rejectionReasonContainer}>
                      <Ionicons name="alert-circle" size={14} color={colors.error} />
                      <Text style={dynamicStyles.rejectionReasonText}>
                        Ablehnungsgrund: {sickLeave.rejection_reason}
                      </Text>
                    </View>
                  )}
                </View>
                ...
                <View style={dynamicStyles.vacationActionButton}>
                  <TouchableOpacity 
                    ...
                    onPress={() => {
                      Alert.alert(
                        '🗑️ Krankmeldung löschen',
                        `Möchten Sie diese Krankmeldung wirklich löschen?\n\nZeitraum: ${sickLeave.start_date} bis ${sickLeave.end_date}\nStatus: ${getStatusText(sickLeave.status)}`,
                        [
                          { text: 'Abbrechen', style: 'cancel' },
                          { 
                            text: 'Löschen', 
                            ...
                            onPress: async () => {
                              try {
                                ...
                                await axios.delete(`${API_URL}/api/sick-leave/${sickLeave.id}`, config);
                                
                                Alert.alert('✅ Erfolg', 'Krankmeldung wurde gelöscht');
                                
                                // Sofortige UI-Aktualisierung
                                setSickLeaves(prev => prev.filter(s => s.id !== sickLeave.id));
                                ...
                                Alert.alert('❌ Fehler', 'Krankmeldung konnte nicht gelöscht werden');
                              }
                            }
                          }
                        ]
      ...
      {/* ✅ NEU: Modern Sick Leave Request Modal */}
      <Modal
          ...
          <View style={dynamicStyles.modalOverlay}>
            <View style={[dynamicStyles.modalContainer, { maxHeight: '80%' }]}>
              ...
              <View style={dynamicStyles.modernModalHeader}>
                ...
                <View style={dynamicStyles.modernModalTitleContainer}>
                  <Text style={dynamicStyles.modernModalTitle}>🏥 Krankmeldung</Text>
                  <Text style={dynamicStyles.modernModalSubtitle}>Neue Krankmeldung einreichen</Text>
                </View>
                <TouchableOpacity
                  style={dynamicStyles.modernModalCloseButton}
              ...
              >
                {/* Date Selection */}
                <View style={dynamicStyles.modernFormSection}>
                  <Text style={dynamicStyles.modernSectionLabel}>📅 Krankheitszeitraum</Text>
                  ...
                  <View style={dynamicStyles.dateInputsContainer}>
                    <View style={dynamicStyles.dateInputWrapper}>
                      ...
                      <View style={dynamicStyles.modernInputContainer}>
                        ...
                        <TextInput
                          style={dynamicStyles.modernInput}
                          value={sickLeaveFormData.start_date}
                          onChangeText={(value) => setSickLeaveFormData({...sickLeaveFormData, start_date: value})}
                          placeholder="YYYY-MM-DD"
                          placeholderTextColor={colors.textMuted}
                        />
                      </View>
                    ...
                    <View style={dynamicStyles.dateInputWrapper}>
                      ...
                      <View style={dynamicStyles.modernInputContainer}>
                        ...
                        <TextInput
                          style={dynamicStyles.modernInput}
                          value={sickLeaveFormData.end_date}
                          onChangeText={(value) => setSickLeaveFormData({...sickLeaveFormData, end_date: value})}
                          placeholder="YYYY-MM-DD"
                          placeholderTextColor={colors.textMuted}
                        />
                      </View>
                ...
                {/* Medical Certificate */}
                <View style={dynamicStyles.modernFormSection}>
                  <Text style={dynamicStyles.modernSectionLabel}>🩺 Ärztliches Attest</Text>
                  <TouchableOpacity 
                    style={[dynamicStyles.modernInputContainer, { 
                      backgroundColor: sickLeaveFormData.medical_certificate ? colors.success + '10' : colors.background 
                    }]}
                    onPress={() => setSickLeaveFormData({
                      ...sickLeaveFormData, 
                      medical_certificate: !sickLeaveFormData.medical_certificate
                    })}
                  >
                    <Ionicons 
                      name={sickLeaveFormData.medical_certificate ? "checkmark-circle" : "ellipse-outline"} 
                      size={20} 
                      color={sickLeaveFormData.medical_certificate ? colors.success : colors.textMuted} 
                    />
                    <Text style={[dynamicStyles.modernInput, { 
                      color: sickLeaveFormData.medical_certificate ? colors.success : colors.text,
                      fontWeight: sickLeaveFormData.medical_certificate ? '600' : '400'
                    }]}>
                      {sickLeaveFormData.medical_certificate ? '✅ Attest liegt vor' : 'Attest liegt vor'}
                    </Text>
                  </TouchableOpacity>
                  <Text style={dynamicStyles.inputHint}>
                ...
                {/* Reason Section */}
                <View style={dynamicStyles.modernFormSection}>
                  <Text style={dynamicStyles.modernSectionLabel}>📝 Grund/Beschreibung</Text>
                  <View style={dynamicStyles.modernInputContainer}>
                    <Ionicons name="document-text-outline" size={20} color={colors.primary} />
                    <TextInput
                      style={[dynamicStyles.modernInput, { 
                      ...
                      value={sickLeaveFormData.reason}
                      onChangeText={(value) => setSickLeaveFormData({...sickLeaveFormData, reason: value})}
                      placeholder="z.B. Erkältung, Migräne, Verletzung..."
                      placeholderTextColor={colors.textMuted}
                      multiline
                      numberOfLines={4}
                    />
                  ...
                  <Text style={dynamicStyles.inputHint}>
                    💡 Eine kurze Beschreibung hilft bei der Bearbeitung
                  </Text>
                </View>
              </ScrollView>
